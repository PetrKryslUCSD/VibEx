function sdof_damped_TF_GEN
    % Visualization of solution of the transfer function (TF) of a damped
    ...
    function Do_it(monoff,dperc,konoff)
        ...
        subplot(2,1,1);
        plot(omega/omega_n,abs(H), 'linewidth', 2, 'color', 'red', 'marker', 'none');
        set(gca,'ylim', [0,0.1+min([13,max(abs(H))])]);
        grid on
        axis on
        xlabel('Frequency ratio \beta');
        ylabel('Dyn. magnif. factor D');
        subplot(2,1,2);
        plot(omega/omega_n,atan2(imag(H),real(H))*180/pi, 'linewidth', 2, 'color', 'red', 'marker', 'none')
        grid on
        axis on
        xlabel('Frequency ratio \beta');
